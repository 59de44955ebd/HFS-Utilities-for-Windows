...
int strmatch(const char *str, const char *pat)
{
	while (1)
	{
		...
		/* fall through */
		default:
			if (hfs_charorder[(unsigned char) *pat] != hfs_charorder[(unsigned char) *str])
				return 0;
		}
...
int doglob(hfsvol *vol, dlist *list, const char *dir, const char *rem)
{
	...
	if (obrace)
	{
		...
		if (cbrace == 0 || dstr_append(&new, rem, obrace - rem) == -1)
		{
			dstr_free(&new);
			return -1;
		}
	...
	if (special)
	{
		...
		if (result == 0 && ! found)
		{
			...
			else
			{
				...
				for (rem = dstr_string(&new) + len, ptr = rem; *rem; ++rem, ++ptr)
				{
					if (*rem == '\\')
						++rem;
...
char **hfs_glob_w(hfsvol *vol, int argc, wchar_t *argv[], int *nelts)
{
	dlist list;
	int i;
	char * macroman;
	...
	for (i = 0; i < argc; ++i)
	{
		macroman = utf16ToMacRoman(argv[i]);
		if (macroman == 0)
		{
			dl_free(&list);
			return 0;
		}
		if (doglob(vol, &list, "", macroman) == -1)
		{
			free(macroman);
			dl_free(&list);
			return 0;
