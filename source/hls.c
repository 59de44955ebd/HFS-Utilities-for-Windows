...
int usage(void)
{
	fwprintf(stderr, L"Usage: ls [options] [hfs-path ...]\n");
...
void showmisc(hfsdirent *ent, int flags)
{
	...
	if (flags & HLS_CATIDS)
		wprintf(L"%7lu ", ent->cnid);
	if (flags & HLS_SIZE)
		wprintf(L"%4lu ", size / 1024 + (size % 1024 != 0));
}
		...
		int flags, int options, int width)
{
	...
	for (i = 0; i < sz; ++i)
	{
		if (i)
		{
			...
			{
			for (len = strlen(strs[i - 1]) + misc; len < maxlen; ++len)
				putwchar(L' ');
			}
		}
...
int showfiles(darray *files, int flags, int options, int width)
{
	...
	if (dl_init(&list) == -1)
	{
		fwprintf(stderr, L"ls: not enough memory\n");
		return -1;
	}
	...
	for (i = 0; i < sz; ++i)
	{
		if (outpath(&str, &ents[i], flags) == -1 || dl_append(&list, dstr_string(&str)) == -1)
		{
			result = -1;
			break;
		}
	...
	for (i = 0; i < dsz; ++i)
	{
		...
		while (hfs_readdir(dir, &ent.dirent) != -1)
		{
			...
			if ((ent.dirent.flags & HFS_ISDIR) && (flags & HLS_RECURSIVE))
			{
				...
				if (strchr(path, ':') == 0 && dstr_append(&str, ":", 1) == -1)
					result = -1;

				if (dstr_append(&str, path, -1) == -1)
					result = -1;

				if (path[strlen(path) - 1] != ':' && dstr_append(&str, ":", 1) == -1)
					result = -1;
		...
		if (flags & HLS_NAME)
		{
			...
			if (wpath != 0)
			{
				wprintf(L"%s%s", wpath, path[strlen(path) - 1] == ':' ? L"\n" : L":\n");
				free(wpath);
			}
		}
 ...
 */
int hls_main(int argc, wchar_t *argv[])
{
	hfsvol *vol;
	int fargc, i;
	char **fargv = 0;
...
# endif
	...
	while (1)
	{
		...
		opt = getopt(argc, argv, L"1abcdfilmqrstxw:CFNQRSU?");
		if (opt == EOF)
			break;
	...
	fargv = hfsutil_glob(vol, argc - optind - 1, &argv[optind +1], &fargc, &result);
