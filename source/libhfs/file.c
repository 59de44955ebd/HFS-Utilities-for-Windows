  ...
  while (start < end)
      ...
      for (i = 0; i < 3; ++i)
	{
	  ...
	  if (start == end)
	    ...
	    __ERROR(EIO, "file extents exceed file physical length");
	  else if (num == 0)
	    __ERROR(EIO, "empty file extent");
	}
...
int f_trunc(hfsfile *file)
{
  ...
  if (newpylen > *pylen)
    __ERROR(EIO, "file size exceeds physical length");
  ...
  do
      ...
      while (dlen && ++i < 3)
	{
	  ...
	  if (num == 0)
	    ...
	    __ERROR(EIO, "file extents exceed physical size");
