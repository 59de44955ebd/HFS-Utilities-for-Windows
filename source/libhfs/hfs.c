...
int getvol(hfsvol **vol)
{
	...
	{
		if (curvol == 0)
			__ERROR(EINVAL, "no volume is current");
 ...
 */
hfsvol *hfs_mount(const wchar_t *path, int pnum, int mode)
{
	hfsvol *vol, *check;
	...
	/* mount the volume */
	if (v_geometry(vol, pnum) == -1 || v_mount(vol) == -1)
		goto fail;
...
int hfs_vsetattr(hfsvol *vol, hfsvolent *ent)
{
	...
	if (ent->clumpsz % vol->mdb.drAlBlkSiz != 0)
		__ERROR(EINVAL, "illegal clump size");

	/* make sure "blessed" folder exists */

	if (ent->blessed &&
			v_getdthread(vol, ent->blessed, 0, 0) <= 0)
		__ERROR(EINVAL, "illegal blessed folder");
...
int hfs_readdir(hfsdir *dir, hfsdirent *ent)
{
	...
	if (dir->dirid == 0)
	{
		...
		if (vol == 0)
			__ERROR(ENOENT, "no more entries");
	...
	while (1)
	{
		...
		switch (data.cdrType)
		{
			...
			default:
				dir->n.rnum = -1;
				__ERROR(EIO, "unexpected directory entry found");
		}
	}
...
hfsfile *hfs_create(hfsvol *vol, const char *path, const char *type, const char *creator)
{
	...
	f_init(file, vol, vol->mdb.drNxtCNID++, name); //file.c
	vol->flags |= HFS_VOL_UPDATE_MDB;
	...
	file->cat.u.fil.filUsrWds.fdType = 	d_getsl((const unsigned char *) type);
	file->cat.u.fil.filUsrWds.fdCreator = 	d_getsl((const unsigned char *) creator);
	...
	if (bt_insert(&vol->cat, record, reclen) == -1 || v_adjvalence(vol, file->parid, 0, 1) == -1)
		goto fail;
...
int hfs_close(hfsfile *file)
{
	...
	if (f_trunc(file) == -1 || f_flush(file) == -1)
		result = -1;
...
int hfs_stat(hfsvol *vol, const char *path, hfsdirent *ent)
{
	...
	if (getvol(&vol) == -1 || v_resolve(&vol, path, &data, &parid, name, 0) <= 0)
		goto fail;
...
int hfs_setattr(hfsvol *vol, const char *path, const hfsdirent *ent)
{
	...
	if (getvol(&vol) == -1 || v_resolve(&vol, path, &data, 0, 0, &n) <= 0)
		goto fail;
...
int hfs_rmdir(hfsvol *vol, const char *path)
{
	...
	if (bt_delete(&vol->cat, pkey) == -1 || v_adjvalence(vol, parid, 1, -1) == -1)
		goto fail;
...
int hfs_rename(hfsvol *vol, const char *srcpath, const char *dstpath)
{
	...
	if (getvol(&vol) == -1 || v_resolve(&vol, srcpath, &src, &srcid, srcname, 0) <= 0)
		goto fail;
	...
	if (vol != srcvol)
		__ERROR(EINVAL, "can't move across volumes");

	if (dstid == 0)
		__ERROR(ENOENT, "bad destination path");
	...
	if (found)
	{
		...
		if (moving || d_relstring(srcname, dstname))
			__ERROR(EEXIST, "can't use destination name");
	}

	/* can't move anything into the root directory's parent */

	if (moving && dstid == HFS_CNID_ROOTPAR)
		__ERROR(EINVAL, "can't move above root directory");
	...
	if (moving && isdir)
	{
		...
		if (src.u.dir.dirDirID == HFS_CNID_ROOTDIR)
			__ERROR(EINVAL, "can't move root directory");
		...
		for (id = dstid; id != HFS_CNID_ROOTDIR; id = dst.u.dthd.thdParID)
		{
			if (id == src.u.dir.dirDirID)
				__ERROR(EINVAL, "can't move directory inside itself");
 ...
 */
int hfs_zero(const wchar_t *path, unsigned int maxparts, unsigned long *blocks)
{
	hfsvol vol;
	...
	if (maxparts < 1)
		__ERROR(EINVAL, "must allow at least 1 partition");

	if (v_open(&vol, path, HFS_MODE_RDWR) == -1 || v_geometry(&vol, 0) == -1)
		goto fail;
	...
	if (blocks)
	{
		...
		if (! found)
			__ERROR(EIO, "unable to determine free partition space");
 ...
 */
int hfs_mkpart(const wchar_t *path, unsigned long len)
{
	hfsvol vol;
 ...
 */
int hfs_nparts(const wchar_t *path)
{
	hfsvol vol;
	int nparts, found;
	Partition map;
 ...
 */
int hfs_format(const wchar_t *path, int pnum, int mode, const char *vname,
				 unsigned int nbadblocks, const unsigned long badblocks[])
{
	...
	if (v_open(&vol, path, HFS_MODE_RDWR) == -1 || v_geometry(&vol, pnum) == -1)
		goto fail;
	...
	if (nbadblocks > 0)
	{
		if (nbadblocks * 4 > vol.vlen)
			__ERROR(EINVAL, "volume contains too many bad blocks");
		...
		for (i = 0; i < nbadblocks; ++i)
		{
			...
			if (bnum < vol.mdb.drAlBlSt || bnum == vol.vlen - 2)
				__ERROR(EINVAL, "can't spare critical bad block");
			else if (bnum >= vol.vlen)
				__ERROR(EINVAL, "bad block not in volume");
	...
	if (nbadblocks > 0)
	{
		...
		qsort(badalloc, nbadblocks, sizeof(*badalloc), (int (*)(const void *, const void *)) compare);
		...
		for (i = 0; i < nbadblocks; ++i)
		{
			if (i == 0 || badalloc[i] != extent.xdrStABN)
			{
				...
				if (extent.xdrStABN < vol.mdb.drNmAlBlks && f_addextent(&bbfile, &extent) == -1)
					goto fail;
			}
		}
	...
	if (vol.flags & HFS_OPT_ZERO)
	{
		...
		for (bnum = vol.mdb.drAlBlSt + vol.mdb.drNmAlBlks * vol.lpa; bnum < vol.vlen - 2; ++bnum)
			b_writelb(&vol, bnum, &b);
