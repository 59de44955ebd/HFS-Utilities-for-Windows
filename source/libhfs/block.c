...
{
  fwprintf(stderr, L"BLOCK: CACHE vol 0x%lx \"%hs\" hit/miss ratio = %.3f\n",
	  (unsigned long) cache->vol,
	  cache->vol->mdb.drVN,
...
void b_dumpcache(const bcache *cache)
{
  ...
  fwprintf(stderr, L"BLOCK CACHE DUMP:\n");

  for (i = 0, b = cache->tail->cnext; i < HFS_CACHESZ; ++i, b = b->cnext)
    {
      if (INUSE(b))
	{
	  fwprintf(stderr, L"\t %lu", b->bnum);
	  if (DIRTY(b))
	    fwprintf(stderr, L"*");

	  fwprintf(stderr, L":%u", b->count);
	}
    }

  fwprintf(stderr, L"\n");

  fwprintf(stderr, L"BLOCK HASH DUMP:\n");
...
# ifdef DEBUG
  if (INUSE(b))
    fwprintf(stderr, L"BLOCK: CACHE reusing bucket containing "
...
# ifdef DEBUG
  fwprintf(stderr, L"BLOCK: READ vol 0x%lx block %lu",
	  (unsigned long) vol, bnum);
  if (blen > 1)
    fwprintf(stderr, L"+%u[..%lu]\n", blen - 1, bnum + blen - 1);
  else
    fwprintf(stderr, L"\n");
# endif
  ...
  if (nblocks != bnum)
    __ERROR(EIO, "block seek failed for read");

  nblocks = os_read(&vol->priv, bp, blen);
  if (nblocks == (unsigned long) -1)
    goto fail;
  if (nblocks != blen)
    __ERROR(EIO, "incomplete block read");
...
# ifdef DEBUG
  fwprintf(stderr, L"BLOCK: WRITE vol 0x%lx block %lu",
	  (unsigned long) vol, bnum);
  if (blen > 1)
    fwprintf(stderr, L"+%u[..%lu]\n", blen - 1, bnum + blen - 1);
  else
...
# endif
  ...
  if (nblocks != bnum)
    __ERROR(EIO, "block seek failed for write");
  ...
  if (nblocks != blen)
    __ERROR(EIO, "incomplete block write");
...
int b_readlb(hfsvol *vol, unsigned long bnum, block *bp)
{
  if (vol->vlen > 0 && bnum >= vol->vlen)
    __ERROR(EIO, "read nonexistent logical block");
...
{
  if (vol->vlen > 0 && bnum >= vol->vlen)
    __ERROR(EIO, "write nonexistent logical block");
...
int b_readab(hfsvol *vol, unsigned int anum, unsigned int index, block *bp)
{
  ...
  if (anum >= vol->mdb.drNmAlBlks)
    __ERROR(EIO, "read nonexistent allocation block");
  else if (vol->vbm && ! BMTST(vol->vbm, anum))
    __ERROR(EIO, "read unallocated block");
	      ...
	      unsigned int anum, unsigned int index, const block *bp)
{
  ...
  if (anum >= vol->mdb.drNmAlBlks)
    __ERROR(EIO, "write nonexistent allocation block");
  else if (vol->vbm && ! BMTST(vol->vbm, anum))
    __ERROR(EIO, "write unallocated block");
...
unsigned long b_size(hfsvol *vol)
{
  ...
  if (b_readpb(vol, 0, &b, 1) == -1)
    __ERROR(EIO, "size of medium indeterminable or empty");
  ...
  if (high == 0)
    __ERROR(EIO, "size of medium indeterminable or too large");
