...
# if 0
  fwprintf(stderr, L"BTREE: GET vol \"%hs\" btree \"%hs\" node %lu\n",
...
# endif
  ...
  if (nnum > 0 && nnum >= bt->hdr.bthNNodes)
    __ERROR(EIO, "read nonexistent b*-tree node");
  else if (bt->map && ! BMTST(bt->map, nnum))
    __ERROR(EIO, "read unallocated b*-tree node");
  ...
  if (np->nd.ndNRecs > HFS_MAX_NRECS)
    __ERROR(EIO, "too many b*-tree node records");
...
# if 0
  fwprintf(stderr, L"BTREE: PUT vol \"%hs\" btree \"%hs\" node %lu\n",
	  bt->f.vol->mdb.drVN, bt->f.name, np->nnum);
# endif
  ...
  if (np->nnum > 0 && np->nnum >= bt->hdr.bthNNodes)
    __ERROR(EIO, "write nonexistent b*-tree node");
  else if (bt->map && ! BMTST(bt->map, np->nnum))
    __ERROR(EIO, "write unallocated b*-tree node");
...
int bt_readhdr(btree *bt)
{
  ...
  if (bt->hdrnd.nd.ndType != ndHdrNode ||
    ...
    __ERROR(EIO, "malformed b*-tree header node");
  ...
  if (bt->hdr.bthNodeSize != HFS_BLOCKSZ)
    __ERROR(EINVAL, "unsupported b*-tree node size");
  ...
  while (nnum)
	...
	goto fail;
	  ...
	  n.roff[0] != 0x00e ||
	  n.roff[1] != 0x1fa)
	__ERROR(EIO, "malformed b*-tree map node");
  ...
  while (mapsz)
    {
      ...
      if (nnum == 0)
	__ERROR(EIO, "truncated b*-tree map");
...
int insertx(node *np, byte *record, int *reclen)
{
  ...
  if (n_search(np, record))
    __ERROR(EIO, "b*-tree record already exists");
  ...
  switch (np->nd.ndType)
    ...
    default:
      __ERROR(EIO, "unexpected b*-tree node");
    }
...
int deletex(node *np, const byte *key, byte *record, int *flag)
{
  ...
  switch (np->nd.ndType)
    ...
    case ndIndxNode:
      if (np->rnum == -1)
	__ERROR(EIO, "b*-tree record not found");
