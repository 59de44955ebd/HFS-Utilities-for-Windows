...
int m_zeropm(hfsvol *vol, unsigned int maxparts)
{
  ...
  if (maxparts < 2)
    __ERROR(EINVAL, "must allow at least 2 partitions");
		  ...
		  Partition *map, unsigned long *start)
{
  ...
  if (start && *start > 0)
	...
	__ERROR(EINVAL, "partition not found");
    }
  else
    bnum = 1;
  ...
  while (1)
      ...
      if (map->pmSig != HFS_PM_SIGWORD)
	{
	  ...
	  if (map->pmSig == HFS_PM_SIGWORD_OLD)
	    __ERROR(EINVAL, "old partition map format not supported");
	  else
	    __ERROR(EINVAL, "invalid partition map");
	}
	     ...
	     const char *name, const char *type, unsigned long len)
{
  ...
  if (strlen(name) > 32 ||
      strlen(type) > 32)
    __ERROR(EINVAL, "partition name/type can each be at most 32 chars");

  if (len == 0)
    __ERROR(EINVAL, "partition length must be > 0");
  ...
  if (! found)
    __ERROR(EIO, "cannot find partition map's partition");
  ...
  do
	...
	goto fail;
      ...
      if (! found)
	__ERROR(ENOSPC, "no available partitions");
    }
  ...
  if (remain && nparts >= maxparts)
    __ERROR(EINVAL, "must allocate all blocks in free space");
