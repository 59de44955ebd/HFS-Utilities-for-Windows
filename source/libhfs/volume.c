 ...
 */
int v_open(hfsvol *vol, const wchar_t *path, int mode)
{
  if (vol->flags & HFS_VOL_OPEN)
    __ERROR(EINVAL, "volume already open");
 ...
 */
int v_same(hfsvol *vol, const wchar_t *path)
{
  ...
  if (pnum == 0)
      ...
      if (map.pmDataCnt)
	{
	  if ((unsigned long) map.pmLgDataStart +
	    ...
	    __ERROR(EINVAL, "partition data overflows partition");
      ...
      if (vol->vlen == 0)
	__ERROR(EINVAL, "volume partition is empty");
    }

  if (vol->vlen < 800 * (1024 >> HFS_BLOCKSZ_BITS))
    __ERROR(EINVAL, "volume is smaller than 800K");
...
int v_readmdb(hfsvol *vol)
{
  ...
  if (vol->mdb.drSigWord != HFS_SIGWORD)
	...
	__ERROR(EINVAL, "MFS volume format not supported");
      else
	__ERROR(EINVAL, "not a Macintosh HFS volume");
    }

  if (vol->mdb.drAlBlkSiz % HFS_BLOCKSZ != 0)
    __ERROR(EINVAL, "bad volume allocation block size");
...
int v_readvbm(hfsvol *vol)
{
  ...
  if (vol->mdb.drAlBlSt - vbmst < vbmsz)
    __ERROR(EIO, "volume bitmap collides with volume data");
...
int v_allocblocks(hfsvol *vol, ExtDescriptor *blocks)
{
  ...
  if (found == 0 || found > vol->mdb.drFreeBks)
    __ERROR(EIO, "bad volume bitmap or free block count");
  ...
  while (1)
	...
	*parid = dirid;
	...
	case cdrFilRec:
	  ...
	  __ERROR(ENOTDIR, "invalid pathname");

	default:
	  __ERROR(EIO, "unexpected catalog record");
	}
    }
...
int v_adjvalence(hfsvol *vol, unsigned long parid, int isdir, int adj)
{
  ...
  if (v_getdthread(vol, parid, &data, 0) <= 0 ||
    ...
    __ERROR(EIO, "can't find parent directory");
...
# ifdef DEBUG
  fwprintf(stderr, L"VOL: \"%hs\" not cleanly unmounted\n",
	  vol->mdb.drVN);
# endif
...
# ifdef DEBUG
  fwprintf(stderr, L"VOL: scavenging...\n");
# endif
  ...
  if (vol->mdb.drFreeBks != blks)
    {
# ifdef DEBUG
      fwprintf(stderr, L"VOL: updating free blocks from %u to %u\n",
...
# endif
  ...
  if ((unsigned long) vol->mdb.drNxtCNID <= lastcnid)
    {
# ifdef DEBUG
      fwprintf(stderr, L"VOL: updating next CNID from %lu to %lu\n",
...
# ifdef DEBUG
  fwprintf(stderr, L"VOL: scavenging complete\n");
# endif
